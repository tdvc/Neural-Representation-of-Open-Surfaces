...
if __name__ == '__main__':
    ...
    model_dir = os.path.join(os.getcwd(),"experiments",os.sys.argv[1],os.getenv(os.sys.argv[2]+'_model_test_dir'))
    ...
    save_model = os.path.join(model_dir,cfg.model_test.name)
    ...
    checkpoint_training_model = torch.load(os.path.join(model_train_dir,cfg.model_train.name),map_location=device)
    training_model_epoch = checkpoint_training_model['epoch']
    training_model_loss = checkpoint_training_model['loss']
    
    # Create new status file
    f = open(save_status_file, "w")
    f.write("Status file for test for experiment " + os.sys.argv[1] + " using " + os.sys.argv[2] + "\n")
    ...
    f.write("The training model has been trained for: " + str(training_model_epoch) + " epochs with loss: " + str(training_model_loss) + "\n")
    f.write("Running script at time: " + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + "\n\n")
    f.close()
    # Copy test script and configuration file
    shutil.copyfile(os.path.join(os.getcwd(),"test.py"), os.path.join(model_dir,"test.py"))
    ...
    # ---------------------------------------------------------------------------- #
    latent_vector_optimizer = optim.Adam([lv.latent_vectors], lr=cfg.lv_optimizer.learning_rate_test) # withs L2 regularization
